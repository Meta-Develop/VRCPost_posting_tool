"""Calendar tab (CustomTkinter).

Calendar UI for visually reviewing jobs by month.
"""
...
class CalendarTab(ctk.CTkFrame):
    ...
    def _build_ui(self) -> None:
        ctk.CTkLabel(
            self, text="Calendar", font=ctk.CTkFont(size=22, weight="bold")
        ).pack(anchor="w", pady=(0, 8))

        # Left/right: Calendar + Details
        ...
        self._detail_title = ctk.CTkLabel(
            detail, text="Select a date", font=ctk.CTkFont(size=14, weight="bold")
        )
    ...
    def _select_day(self, d: date) -> None:
        ...
        if not day_jobs:
            ctk.CTkLabel(
                self._detail_scroll, text="No jobs", text_color="gray"
            ).pack(pady=20)
            return
